...
class Router(ABC, ContextDependent):
    ...
    Args:
        ...
        mnemo_mcp_agents_names: A list of server names to route the input to.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.server_names and not self.agents and not self.functions:
            raise ValueError(
                "At least one of mnemo_mcp_agents_names, agents, or functions must be provided."
            )

        if self.server_names and not self.server_registry:
            raise ValueError(
                "server_registry must be provided if mnemo_mcp_agents_names are provided."
            )
