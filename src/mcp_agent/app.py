...
from mnemo_agent.context import Context, initialize_context, cleanup_context
from mnemo_agent.config import Settings
from mnemo_agent.event_progress import ProgressAction
...
from mnemo_agent.human_input.handler import console_input_callback
...
class MCPApp:
    ...
    def __init__(
        ...
        Args:
            ...
            settings: Application configuration - If unspecified, the settings are loaded from mnemo_agent.config.yaml.
    ...
    def logger(self):
        if self._logger is None:
            ...
            self._logger = get_logger(f"mnemo_agent.{self.name}", session_id=session_id)
        return self._logger
