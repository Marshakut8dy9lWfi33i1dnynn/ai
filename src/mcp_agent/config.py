...
class Settings(BaseSettings):
    ...
    def find_config(cls) -> Path | None:
        ...
        return cls._find_config(["mcp-agent.config.yaml", "mnemo_agent.config.yaml"])

    @classmethod
    def find_secrets(cls) -> Path | None:
        """Find the secrets file in the current directory or parent directories."""
        return cls._find_config(["mcp-agent.secrets.yaml", "mnemo_agent.secrets.yaml"])
...
def get_settings(config_path: str | None = None) -> Settings:
    ...
    if config_file and config_file.exists():
        ...
        secrets_found = False
        for secrets_filename in ["mcp-agent.secrets.yaml", "mnemo_agent.secrets.yaml"]:
