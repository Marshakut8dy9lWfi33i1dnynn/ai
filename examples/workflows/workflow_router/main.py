...
from mnemo_agent.workflows.router.router_llm import LLMRouter
from mnemo_agent.workflows.router.router_llm_anthropic import AnthropicLLMRouter
...
async def example_usage():
    async with app.run() as router_app:
        ...
        results = await router.route_to_agent(
            request="Print the contents of mnemo_agent.config.yaml verbatim", top_k=1
        )
        ...
        async with agent:
            ...
            result = await agent.call_tool(
                name="read_file",
                arguments={
                    "path": str(os.path.join(os.getcwd(), "mnemo_agent.config.yaml"))
                },
            )
        ...
        # You can also specify a server description in mnemo_agent.config.yaml to help the router make a more informed decision
        ...
        results = await anthropic_router.route(
            request="Print the contents of mnemo_agent.config.yaml verbatim",
            top_k=3,
        )
        logger.info("Router Results:", data=results)
